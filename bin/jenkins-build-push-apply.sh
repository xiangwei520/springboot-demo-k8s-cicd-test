...
ssh root@192.168.109.102 'docker rmi $(docker images | grep "springboot-demo-k8s-cicd-test" | awk '{print $3}') ; echo "docker rmi old images ok!" ; docker build -t registry.cn-hangzhou.aliyuncs.com/xiangwei-public/springboot-demo-k8s-cicd-test:1.0.6-SNAPSHOT /opt/springboot/. ; echo "docker build ok!" ; docker push registry.cn-hangzhou.aliyuncs.com/xiangwei-public/springboot-demo-k8s-cicd-test:1.0.6-SNAPSHOT ; echo "docker push ok!"  ; docker rmi registry.cn-hangzhou.aliyuncs.com/xiangwei-public/springboot-demo-k8s-cicd-test:1.0.6-SNAPSHOT ; echo "docker rmi new images ok!" ; kubectl delete -f /opt/springboot/springboot-demo.yaml ; echo "kubectl delete springboot-demo.yaml ok!" ; kubectl apply -f /opt/springboot/springboot-demo.yaml ; echo "kubectl apply springboot-demo.yaml ok!" ; rm -rf /opt/springboot/springboot-demo-k8s-cicd-test-1.0.6-SNAPSHOT.jar ; rm -rf /opt/springboot/Dockerfile'
